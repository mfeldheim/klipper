...
class ClockSync:
    ...
    def _handle_clock(self, params):
        with self.lock:
            ...
            if (clock_diff2 > 25. * self.prediction_variance and
                    clock_diff2 > (.000500 * self.mcu_freq) ** 2):
...
class SecondarySync(ClockSync):
    ...
    def dump_debug(self):
        adjusted_offset, adjusted_freq = self.clock_adj
        return f"{super().dump_debug()} clock_adj=({adjusted_offset:.3f} {adjusted_freq:.3f})"
    def stats(self, eventtime):
        adjusted_offset, adjusted_freq = self.clock_adj
    ...
    def calibrate_clock(self, print_time, eventtime):
        with self.lock:
            ...
            # Estimate the current print time of the main MCU
